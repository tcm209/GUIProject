...
class GraphUtils(object):
    ...
    def draw_graph(self,input_width,input_height,horizontal_num,vertical_num,main_pipe_w,inner_rod_w,inner_pipe_w,extend_w,category,out_line_spec,inner_rod_spec,inner_pipe_spec):
        ...
        #
        self.y_max = input_height*self.multiple+500
        self.x_max = input_width*self.multiple+500
        ...
        mm_h_num_extend_h=h_num_extend_h*self.multiple
        ...
        for i in range(0,vertical_num):
            ...
            mm_point_x=point_x*self.multiple
            if i == 0:
                ...
                # 垂直红线
                plt.plot([-100, -100], [0, vertical_space*self.multiple-self.w_min], color=self.dashed_color_v, linewidth=0.5, linestyle="-")  # y=0.1      画垂直线 水平间距

                plt.plot([-200, -200], [0-self.w_min, input_height*self.multiple], color=self.dashed_color_v, linewidth=0.5,linestyle="-")  # y=0.1      画垂直线 水平间距
                ...
                plt.text(-250, mm_h_num_extend_h / 2-100,
                         "L=" + str(round(input_height*self.multiple,2)) + "mm",
                         ha='left',
                         va='bottom',
                         fontdict=self.font_style,
                         rotation=self.v_rotation
                ...
                if category == "竖式":
                    plt.plot([mm_point_x, mm_point_x], [0, input_height*self.multiple], color=self.vertical_pipe_color, linewidth=self.vertical_pipe_bold, linestyle="-", label='规格：S= '+inner_pipe_spec+"； Number = "+str(self.inner_pipe_count-2))
                elif category == "横式":
                    plt.plot([mm_point_x, mm_point_x], [0, input_height*self.multiple], color=self.vertical_pipe_color,linewidth=self.vertical_pipe_bold, linestyle="-", label='规格：S= '+inner_rod_spec+'； Number = '+str(self.inner_rod_count-2))
                ...
                #  y=3虚线
                plt.plot([0, -250], [input_height*self.multiple+self.w_min, input_height*self.multiple+self.w_min], color=self.dashed_color, linewidth=0.5, linestyle="--")
                #  y=3.02虚线
                ...
                plt.plot([mm_point_x, mm_point_x], [0, input_height*self.multiple], color=self.vertical_pipe_color, linewidth=self.vertical_pipe_bold, linestyle="-")

        #画水平线
        for i in range(0, horizontal_num):
            ponit_y = i * vertical_space
            mm_point_y=ponit_y*self.multiple
            if i == 0:
                plt.plot([0, input_width*self.multiple], [mm_point_y-self.w_min, mm_point_y-self.w_min], color=self.mian_pipe_color, linewidth=self.main_pipe_bold, linestyle="-")
                #水平蓝虚线
                plt.plot([0, horizontal_space*self.multiple-self.w_min], [-100, -100], color=self.dashed_color_h, linewidth=0.5, linestyle="-")  # 画水平线
                plt.text(horizontal_space*self.multiple / 2, -100,
                ...
                plt.plot([0, input_width*self.multiple - 1.5*self.w_min], [-200, -200], color=self.dashed_color_h, linewidth=0.5,
                         linestyle="-")
                plt.text(input_width*self.multiple / 2, -400,
                         "L=" + str(round(input_width*self.multiple,2)) + "mm",
                         ha='center',
                         va='bottom',
                         fontdict=self.font_style,
                         rotation=self.h_rotation
                ...
                plt.plot([0, input_width*self.multiple ], [-400, -400], color=self.mian_pipe_color, linewidth=0.5,
                         linestyle="-")
                plt.text(input_width*self.multiple / 2-100, -200,
                         "L=" + str(round(input_width*self.multiple-20,2)) + "mm",
                         ha='center',
                         va='bottom',
                         fontdict=self.font_style,
                         rotation=self.h_rotation
                ...
                if category == "竖式":
                    plt.plot([0, input_width*self.multiple], [mm_point_y, mm_point_y], color=self.horizontal_pipe_color, linewidth=self.horizontal_pipe_bold, linestyle="-",label='规格：S= '+inner_rod_spec+'； Number = '+str(self.inner_rod_count-2))
                elif category == "横式":
                    plt.plot([0, input_width*self.multiple], [mm_point_y, mm_point_y], color=self.horizontal_pipe_color, linewidth=self.horizontal_pipe_bold, linestyle="-", label='规格：S= '+inner_pipe_spec+'； Number = '+str(self.inner_pipe_count-2))
                plt.plot([-150, 0], [mm_point_y, mm_point_y], color=self.dashed_color, linewidth=0.5, linestyle="--")  # 延长垂直方向线
            elif i == h_num:
                # 正常最后一根线
                plt.plot([0, input_width*self.multiple], [mm_point_y, mm_point_y], color=self.mian_pipe_color, linewidth=self.main_pipe_bold, linestyle="-")
                # 垂直延长线

                plt.plot([input_width*self.multiple+self.w_min, input_width*self.multiple+self.w_min], [-450, 0-self.w_min], color=self.dashed_color, linewidth=0.5, linestyle="--")  # 延长垂直方向线
                plt.plot([input_width*self.multiple-self.w_min, input_width*self.multiple-self.w_min], [-300, 0-self.w_min], color=self.dashed_color, linewidth=0.5, linestyle="--")  # 延长垂直方向线

            else:
                plt.plot([0, input_width*self.multiple], [mm_point_y, mm_point_y], color=self.horizontal_pipe_color, linewidth=self.horizontal_pipe_bold, linestyle="-")
    ...
    def get_filename(self,mode_w,mode_h):
        now = int(round(time.time() * self.multiple))
        nowformate=time.strftime('%Y-%m-%d',time.localtime(now/self.multiple))
    ...
    def get_space_length(self,input_length,out_line_length,num,inner_pipe_length):
        ...
        dividend = num
        space_length=self.multiple*(divisor/dividend)
        return space_length
...
if __name__=="__main__":
    graph=GraphUtils()
    graph.draw_graph(10.41 ,6.6 ,15 ,7, 0.025 ,0.025 ,0.019, 0.02 ,'横式',' 25 X 25' ,'25 X 25', '方形 19 X 19')
